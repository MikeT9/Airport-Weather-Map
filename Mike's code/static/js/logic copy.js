...
function getCurWx() {
  $.ajax({
    type: 'GET',
    url: `/currentweather/${response[i].LATITUDE}/${response[i].LONGITUDE}`,
    dataType: "text",
    success: function (data) {
      //  console.log(data);
      //  return data;
      marker.bindPopup("<h3>" + airportName + "</h3><hr><p>Temp: " + data.main.temp + "</p>");
      markers.push(marker)
    }
  });
}

function makePopUp(name, marker) {
  marker.onClick(("<h3>" + name + "</h3>"));
  return marker
}

airportData = [];
var dataPromise = d3.csv("static/data/airports.csv")
...
//     // marker.bindPopup("<h3>" + response[i].AIRPORT + "</h3><hr><p>" + new Date(feature.properties.time) + "</p>"); //Add weather here
//     var airportName = response[i].AIRPORT
//     makePopUp(airportName, marker)
//   }
//   createMap(markers);
